...
router.get("/categories", async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT DISTINCT c.mcategory_id, c.mcategory_name
      FROM e_pos_item i
      JOIN e_pos_item_maincategory c
        ON i.mcategory_id = c.mcategory_id
      JOIN e_pos_item_price p
        ON i.item_id = p.item_id
      WHERE p.branch_id = 1
      ORDER BY c.mcategory_name ASC
    `);
...
router.get("/shopcategories", async (req, res) => {
  try {
    ...
    rows.forEach(row => {
      ...
      if (row.scategory_id && !mainCat.subcategories.some(s => s.id === row.scategory_id)) {
