    ...
    <div class="bg-white py-10 px-5 lg:px-24">
      ...
      <div class="mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 items-start font-[Poppins]">
        ...
        <div class="flex gap-0">
          <div class="relative w-full h-[380px] md:h-[466px] lg:h-[500px] bg-[#C7C7C7] overflow-hidden flex items-center justify-center">
            <img :src="product.item_image_1" :alt="product.item_name"
                 class="object-cover w-[300px] md:h-[350px] h-[330px]" />
            <img src="../assets/product/heart.png" alt="Heart" class="absolute top-4 right-4 w-8 h-8 bg-white rounded-full shadow p-1" />
            <img src="../assets/product/search.png" alt="Search" class="absolute top-16 right-4 w-8 h-8 bg-white rounded-full shadow p-1" />
          </div>
        </div>
        ...
        <div>
          ...
          <div class="flex items-center mb-2">
            <span class="font-extrabold text-lg mr-4">Rs.{{ formatPrice(displayedPrice) }}</span>
          </div>

          <p class="text-black mb-4 lg:text-[13px]">{{ product.item_short_description }}</p>

          <!-- Variations -->
          <div v-for="attr in allAttributes" :key="attr.attribute_id" class="mb-4">
            <p class="font-semibold mb-1">{{ attr.attribute_name }}:</p>
            <div class="flex gap-2 flex-wrap">
              <label v-for="varItem in attr.variations" :key="varItem.p1variation_id" class="flex items-center gap-1 cursor-pointer">
          ...
          <div class="flex flex-col sm:flex-row items-center sm:justify-start w-full sm:max-w-xs mb-4 gap-4">
            <div class="flex border border-[#A1A1A1] rounded overflow-hidden bg-white">
              <button @click="decrement" class="w-12 h-12 flex items-center justify-center text-2xl font-semibold text-gray-700 hover:bg-gray-100 border-r border-[#A1A1A1] disabled:opacity-50" :disabled="quantity <= 1" type="button">-</button>
              <input type="number" :value="quantity" class="w-12 h-12 text-center border-0 font-medium" readonly />
              <button @click="increment" class="w-12 h-12 flex items-center justify-center text-2xl font-semibold text-gray-700 hover:bg-gray-100 border-l border-[#A1A1A1]" type="button">+</button>
            </div>
          </div>
          ...
          <div class="flex justify-center sm:justify-start w-full">
            <button
              class="h-12 px-[28px] bg-secondary text-white font-semibold rounded flex items-center justify-center transition hover:bg-gray-800"
              :class="{ 'opacity-50 pointer-events-none': currentStock <= 0 }"
              @click="addToCartSession"
            >
              ADD TO CART
            </button>
          </div>
      ...
      <div class="md:block hidden">
        <div class="flex justify-center space-x-8 border-b border-gray-200 mb-4 mt-8 text-sm lg:text-[15px] font-bold">
          <button v-for="tab in tabs" :key="tab.name" @click="activeTab = tab.name" :class="[ 'pb-2 transition-colors', activeTab === tab.name ? 'border-b-2 border-black text-black' : 'text-gray-500 hover:text-black']">
            {{ tab.label }}
          </button>
        </div>
        <div v-if="activeTab === 'Description'" class="mt-4">
          <p class="font-bold mb-3">Product Information:</p>
          <p class="text-sm text-gray-700">{{ product.item_long_description }}</p>
        </div>
      </div>
      ...
      <section v-if="relatedProducts.length" class="py-10 px-5 lg:px-24">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div v-for="prod in relatedProducts" :key="prod.item_id" class="bg-white shadow rounded p-4 flex flex-col items-center">
            <img :src="prod.item_image_1" :alt="prod.item_name" class="h-40 object-contain mb-2" />
            <h3 class="text-sm font-medium text-center">{{ prod.item_name }}</h3>
            <p class="font-bold text-lg">Rs. {{ formatPrice(prod.item_sell) }}</p>
            <button class="mt-2 bg-secondary text-white px-4 py-2 rounded" @click="goToProduct(prod.item_id)">View</button>
          </div>
        </div>
      </section>
    </div>
  ...
  product.value.prices.forEach(price => {
    price.attributes.forEach(attr => {
      ...
      attr.variations.forEach(v => {
        if(!map[attr.attribute_id].variations.find(x=>x.p1variation_id===v.p1variation_id)) map[attr.attribute_id].variations.push(v);
      });
    });
  });
...
watch(selectedVariations, () => {
  ...
  for(const price of product.value.prices) {
    ...
    if(isMatch) { matched = price; break; }
  }
  if(matched) {
...
watch(() => route.params.id, async id => {
  ...
  try {
    ...
    allAttributes.value.forEach(attr => {
      if(attr.variations?.length) selectedVariations.value[attr.attribute_id] = attr.variations[0].p1variation_id;
    });

    if(product.value.subcategory_id) {
      const relatedRes = await api.get(`products/related/${product.value.subcategory_id}/${product.value.item_id}`);
...
function addToCartSession() {
  ...
  // Determine item_prid (variation/price ID) based on selected variations
  let item_prid = null;
  if(product.value.prices?.length) {
    for(const price of product.value.prices) {
      ...
      if(isMatch) { item_prid = price.item_prid; break; }
    }
  }
  ...
  if(index >= 0) {
    cart[index].quantity += quantity.value; // Same variation: increase quantity
  } else {
    cart.push({
      id: product.value.item_id,
      item_prid: item_prid,        // Store price/variation ID
