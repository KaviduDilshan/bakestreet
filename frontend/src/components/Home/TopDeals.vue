  ...
  <section class="md:px-20 px-5 bg-white">
    ...
    <div class="pt-10 pb-10 text-text-color md:block hidden">
      ...
      <Swiper
        class="w-full"
        :modules="[Navigation]"
        :navigation="{ nextEl: '#topdeals-swiper-next', prevEl: '#topdeals-swiper-prev' }"
      ...
      >
        <SwiperSlide v-for="card in pros" :key="card.item_id">
          ...
          >
            ...
            <!-- ✅ Price -->
            <div
              class="text-[14px] text-text-color font-[700] font-quicksand pt-3"
            >
              Price: {{ card.latest_price }} LKR
            </div>
            <!-- ✅ Sold -->
            <div
              class="text-[14px] text-text-color font-[700] font-quicksand pt-2"
            >
              Sold: {{ card.total_sold }}
            </div>
            <!-- ✅ Add To Cart -->
            <button
              :disabled="card.total_count <= 0"
...
<div class="block md:hidden py-10">
  ...
  <div class="grid grid-cols-2 gap-6">
    ...
    >
      ...
      />
      <p
        class="text-sm text-text-color font-bold mb-1 leading-tight px-1"
      >
        {{ card.item_name }}
      </p>
