    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 lg:gap-6">
      ...
      <div class="col-span-2 bg-[#F5F5F5]">
        <div class="hidden md:grid grid-cols-4 gap-4 px-6 py-4 font-semibold">
          <span>Products</span><span>Price</span><span>Quantity</span
          ><span>Subtotal</span>
        </div>
        ...
        >
          <!-- Product -->
          <div
            class="flex flex-col items-center md:flex-row md:items-center gap-2 md:gap-4 w-full md:w-auto"
          >
            <img
              :src="product.image"
              :alt="product.name"
      ...
      <div class="bg-[#F5F5F5] mt-2 lg:mt-0 w-full p-6 lg:ml-0 md:h-[270px]">
        <div class="flex justify-between mb-3">
          <span>Subtotal</span><span>Rs.{{ formatPrice(subtotal) }}</span>
        </div>
        <div class="flex justify-between mb-3">
        ...
        <div
          class="flex justify-between font-semibold text-lg mt-4 border-t border-[#EEEEEE] pt-4"
        >
        ...
        <button
          ...
          class="w-full mt-6 bg-secondary text-white py-2 rounded font-medium hover:bg-red-700 transition"
        >
          PROCEED TO CHECKOUT
        </button>
        <!-- </router-link> -->
...
const { initAuth, getAuthData } = useAuth();
const products = ref([]);
...
const processToCheckout = async () => {
  if (customerId.value) {
    ...
    localStorage.setItem("nextHiddenPath", 2);
    router.push("/login");
  }
};
...
onMounted(async () => {
  ...
  customerId.value = authData ? authData.customerId : null;
});
</script>
