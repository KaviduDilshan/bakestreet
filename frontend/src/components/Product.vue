  ...
  <div class="flex flex-col lg:flex-row gap-6 px-5 md:px-20 py-10">
    ...
    <aside class="w-full lg:w-1/4 space-y-6">
      ...
      <div
        ...
        @click.away="mobileSortOpen = false"
      >
      ...
      <div class="bg-white p-5 rounded-lg shadow space-y-4">
        ...
        <div v-for="cat in shopCategories" :key="cat.id">
          <div
            ...
            :class="{ 'text-primary font-bold': Number(selectedMainCategory) === Number(cat.id) }"
            @click="toggleMainCategory(cat.id)"
          >
          ...
          >
            ...
            >
              <div
                class="flex justify-between items-center hover:text-primary cursor-pointer"
                :class="{ 'text-primary font-bold': Number(selectedSubcategory) === Number(sub.id) }"
              ...
              <!-- Sub-subcategories -->
              <ul
                v-if="expandedSub[sub.id] && sub.subsubcategories?.length"
                class="pl-4 text-xs space-y-1 text-gray-700"
              >
                <li
                  v-for="ssub in sub.subsubcategories"
                  :key="ssub.id"
                  class="cursor-pointer hover:text-primary"
                  :class="{ 'text-primary font-bold': Number(selectedSubSubcategory) === Number(ssub.id) }"
                  @click.stop="selectSubSubcategory(Number(ssub.id))"
                >
                  {{ ssub.title }}
                  (<span>{{ subSubcategoryCounts[Number(ssub.id)] || 0 }}</span>)
                </li>
              </ul>
            </li>
          </ul>
    ...
    <main class="w-full lg:w-3/4">
      ...
      <div v-else-if="paginatedPros.length === 0" class="text-center py-10">
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        >
          ...
          <p class="text-black font-bold text-sm mb-2">
            Rs.
            {{
              Number(card.min_price).toLocaleString(undefined, {
                minimumFractionDigits: 2,
              })
            }}
            –
            {{
              Number(card.max_price).toLocaleString(undefined, {
                minimumFractionDigits: 2,
              })
            }}
          </p>
          ...
          />
          <router-link
            :to="{ name: 'single', params: { id: encryptId(card.item_id) } }"
...
  
  // 1. DECRYPT the IDs read from the URL query
  const rawCategoryId = route.query.category;
  const rawSubcategoryId = route.query.subcategory;
  
  // Use decryptId only if the value exists
  const categoryId = rawCategoryId ? decryptId(rawCategoryId) : null;
  const subcategoryId = rawSubcategoryId ? decryptId(rawSubcategoryId) : null;

  
  if (categoryId && dataLoaded.value) {
    // Convert the decrypted string ID to a Number for comparison
...
    
    // Set main category
    selectedMainCategory.value = numCategoryId;
    
    // Collapse all
    Object.keys(expanded.value).forEach((id) => {
      expanded.value[Number(id)] = false;
    });
    Object.keys(expandedSub.value).forEach((id) => {
      expandedSub.value[Number(id)] = false;
    });
    
    // Expand selected main category
    expanded.value[numCategoryId] = true;
    
    // Reset pagination
    currentPage.value = 1;
  } else if (subcategoryId && dataLoaded.value) {
    // Convert the decrypted string ID to a Number for comparison
    const numSubcategoryId = Number(subcategoryId);
    
    // Find which main category this subcategory belongs to
    let parentMainCategoryId = null;
    for (const cat of shopCategories.value) {
      // Ensure cat.id is a number for strict comparison
      const foundSub = cat.subcategories?.find(sub => Number(sub.id) === numSubcategoryId);
      if (foundSub) {
        parentMainCategoryId = Number(cat.id);
        break;
      }
    }
    
    if (parentMainCategoryId) {
      // Set both main and sub category
      selectedMainCategory.value = parentMainCategoryId;
      selectedSubcategory.value = numSubcategoryId;
      selectedSubSubcategory.value = null;
      
      // Collapse all first
      Object.keys(expanded.value).forEach((id) => {
...
      });
      
      // Expand parent main category and selected subcategory
      expanded.value[parentMainCategoryId] = true;
      expandedSub.value[numSubcategoryId] = true;
      
      // Reset pagination
      currentPage.value = 1;
    }
  } else if (!categoryId && !subcategoryId) {
    // Reset all if no category or subcategory
...
const filteredPros = computed(() => {
  ...
  if (selectedMainCategory.value) {
    products = products.filter(
      (i) => Number(i.mcategory_id) === Number(selectedMainCategory.value)
    );
  }
  
  if (selectedSubcategory.value) {
    products = products.filter(
      (i) => Number(i.scategory_id) === Number(selectedSubcategory.value)
    );
  }
  
  if (selectedSubSubcategory.value) {
    products = products.filter(
      (i) => Number(i.spcategory_id) === Number(selectedSubSubcategory.value)
    );
  }
  
  products = products.filter(
    (i) =>
      Number(i.max_price) >= rangeData.value.minprice &&
      Number(i.min_price) <= rangeData.value.maxprice
  );
...
const paginatedPros = computed(() => {
  ...
  return filteredPros.value.slice(start, start + itemsPerPage);
});
...
const categoryCounts = computed(() => {
  ...
  const items = selectedMainCategory.value
    ? pros.value.filter((i) => Number(i.mcategory_id) === Number(selectedMainCategory.value))
    : pros.value;

  items.forEach((i) => {
    if (i.scategory_id) {
      const key = Number(i.scategory_id);
...
const subSubcategoryCounts = computed(() => {
  ...
  // Get all products that match the current main and sub category filters
  const items = pros.value.filter((product) => {
    // Must match main category if one is selected
    const isMainMatch =
      !selectedMainCategory.value ||
      Number(product.mcategory_id) === Number(selectedMainCategory.value);
    
    // Must match subcategory if one is selected
    const isSubMatch =
      !selectedSubcategory.value ||
      Number(product.scategory_id) === Number(selectedSubcategory.value);
    
    return isMainMatch && isSubMatch && product.spcategory_id;
  });

  // Count products for each sub-subcategory
  items.forEach((product) => {
    const key = Number(product.spcategory_id);
    counts[key] = (counts[key] || 0) + 1;
  });
...
const resultsText = computed(() => {
  const total = pros.value.length;
  const showing = filteredPros.value.length;
  let categoryName = "All Items";

  if (selectedSubSubcategory.value) {
    const main = shopCategories.value.find(
      (c) => Number(c.id) === Number(selectedMainCategory.value)
    );
    const sub = main?.subcategories?.find(
      (s) => Number(s.id) === Number(selectedSubcategory.value)
    );
    const ssub = sub?.subsubcategories?.find(
      (ss) => Number(ss.id) === Number(selectedSubSubcategory.value)
    );
...
const fetchShopCategories = async () => {
  try {
    ...
    shopCategories.value.forEach((cat) => {
      expanded.value[Number(cat.id)] = false;
    });
  } catch (err) {
    console.error(err);
...
// -------------------- Toggle Functions --------------------
function toggleMainCategory(id) {
  const numId = Number(id);
  
  if (selectedMainCategory.value === numId) {
    expanded.value[numId] = !expanded.value[numId];
    if (!expanded.value[numId]) {
      selectedMainCategory.value = null;
      selectedSubcategory.value = null;
      selectedSubSubcategory.value = null;
    }
    ...
    Object.keys(expandedSub.value).forEach((key) => {
      expandedSub.value[Number(key)] = false;
    });
    expanded.value[numId] = true;
    selectedMainCategory.value = numId;
    selectedSubcategory.value = null;
    selectedSubSubcategory.value = null;
  }
}
...
function toggleSub(subId) {
  ...
  if (selectedSubcategory.value === numSubId) {
    expandedSub.value[numSubId] = !expandedSub.value[numSubId];
    if (!expandedSub.value[numSubId]) {
      selectedSubcategory.value = null;
      selectedSubSubcategory.value = null;
    }
  } else {
    ...
    expandedSub.value[numSubId] = true;
    selectedSubcategory.value = numSubId;
    selectedSubSubcategory.value = null;
  }
}

function selectSubSubcategory(ssubId) {
  selectedSubSubcategory.value = Number(ssubId);
}
...
onMounted(async () => {
  ...
  // Apply category filter after data loads
  await nextTick();
  applyCategoryFilter();
});
</script>
